...
export default function Seguranca() {
  ...
  const [panicPressed, setPanicPressed] = React.useState(false)

  const share = async () => {
    const profile = JSON.parse(localStorage.getItem('cb_profile')||'{}')
    const text = `SOS 🚨\nSou ${profile.nome||'vizinho'} no bairro ${profile.bairro||''}. Preciso de ajuda! Minha localização: ${location.href}`
    if (navigator.share) {
    ...
    <div className="p-4 space-y-4">
      ...
      {helpOpen && (
        <div className="p-3 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
          ...
          <form className="grid grid-cols-1 gap-2" onSubmit={(e)=>{e.preventDefault(); toast('Contatos salvos no dispositivo.'); setHelpOpen(false)}}>
            {[1,2,3].map(i => <input key={i} className="rounded border p-2 bg-white dark:bg-slate-800" placeholder={`Nome + WhatsApp #${i}`} />)}
            <button className="rounded bg-brand text-white py-2">Salvar</button>
          </form>
        </div>
      )}
    </div>
